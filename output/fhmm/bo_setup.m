...
savepath='../output/fhmm/'; % choose where to save the figures and outputs
iem=1; % =1, electromagnetic run; =0, electrostatic run; =2, Darwin model; =3 fluid model

if(iem~=3) % only needed for kinetic version
  N=2; % Number harmonics, enlarge N to make sure the results are convergent
  ...
  ifluidmodel=1; % choose fluid closure model: =1, Xie2014; =2 double polytrope
  % set the default polytrope exponents
  % (gammazs,gammaps)=(1.0,1.0) isothermal; =(3.0,2.0) CGL model
  gammas0=2*[1.0,1.0]; % default [para,perp] polytrope exponents gamma_{para,perp}
%   gammas0=[3.0,2.0];
...
par=zeros(6,1);
par(1)=1.0; % k, =sqrt(kx^2+kz^2), normalized by kn
par(2)=30.0; % theta, the angle between k and B0, normalized by *pi/180
...
% You can scan other parameters by modify line xxx in 'bo_kernel.m',
...
ilogb=0;
pa1=0.01; pa2=1.0/1; dpa=0.01; % 1st parameter a, depends on ipa
% pa1=0; pa2=90; dpa=1; % 1st parameter a, depends on ipa
