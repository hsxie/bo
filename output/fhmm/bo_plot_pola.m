...
if(icase==1)
    close all;
    load(['out_bo_S=2_B0=1e-07_fluid.mat']);
    h=figure('unit','normalized','Position',[0.01 0.05 0.8 0.8],...
        'DefaultAxesFontSize',15);
    linestr={'-','--'};
else
    load(['out_bo_S=2_J=4_N=1_B0=1e-07.mat']);
    linestr={':','-.'};
end

jpl=jpll(icase);
% jpl=1; % select which solution to plot
% pkzz=Pola(:,:,jpl,npolaf-2)/kn;
pkk=Pola(:,:,jpl,npolaf-1)/kn;
...
jjpola=[npolaf,(npolaf+3+npolas*(1-1)+4):(npolaf+3+npolas*(1-1)+7),...
    (npolaf+3+npolas*(2-1)+4):(npolaf+3+npolas*(2-1)+7),1:3];
...
for jj=1:min(length(jjpola),20)
    ...
    hold on;
    if(jjpola(jj)==npolaf)
      plot(pkk,real(Pola(:,:,jpl,jjpola(jj))./wn),linestr{1},...
        pkk,imag(Pola(:,:,jpl,jjpola(jj))./wn),linestr{2},'linewidth',2);
    if(icase==2)
...
%         title(['vdsz/c=',num2str(vdsz/sqrt(c2),3)]);
      ...
      plot(pkk,real(Pola(:,:,jpl,jjpola(jj))),linestr{1},...
        pkk,imag(Pola(:,:,jpl,jjpola(jj))),linestr{2},'linewidth',2);
    end
    grid on;
    xlabel('k/k_n');
...
print(gcf,'-dpng',['bo_plot_pola_iem=',num2str(iem),...
