% 18-10-06 07:05 Hua-sheng XIE, huashengxie@gmail.com, CCF-ENN, China
...
%   Pola=zeros(npa,npb,npl,30+7*S); % 20-11-15 16:23
  Pola=zeros(npa,npb,npl,npolaf+3+npolas*S); % 20-12-24 20:46
  wws2=wws;
  icalp=1; % tell kernel.m which can calculate polarization now
  for jpl=1:npl1
    % in eigs(), use wws(jpa,jpb,jpl) as intitial guess for each (pa,pb)
    run ../modules/bo_kernel; % to update
  end
end
...
print(gcf,'-dpng',[savepath,'fig_bo_',figstr,'_pola.png']);
% savefig([savepath,'fig_bo_',figstr,'_pola.fig']);

end

if(iem~=3)
  filename=['out_bo_S=',num2str(S),'_J=',num2str(J),...
    '_N=',num2str(N),'_B0=',num2str(B0),'.mat'];
else
  filename=['out_bo_S=',num2str(S),'_B0=',num2str(B0),'_fluid.mat'];
end
